...
import 'package:s2toperational/Screens/health_screening_details/controllers/audio_screening_patient_list_controller.dart';
import 'package:s2toperational/Screens/health_screening_details/models/patient_list_model.dart';
import 'audio_screening_form_screen.dart';

class AudioScreeningPatientListScreen extends StatefulWidget {
  final int campID;

  const AudioScreeningPatientListScreen({super.key, required this.campID});

  @override
  State<AudioScreeningPatientListScreen> createState() =>
      _AudioScreeningPatientListScreenState();
}

class _AudioScreeningPatientListScreenState
    extends State<AudioScreeningPatientListScreen> {
  late final AudioScreeningPatientListController controller;

  @override
  void initState() {
    super.initState();
    controller = Get.put(AudioScreeningPatientListController());
    controller.loadData(campId: widget.campID);
  }

  @override
  void dispose() {
    Get.delete<AudioScreeningPatientListController>();
    super.dispose();
  }
  ...
  void _onRowTapped(UserAttendancesUsingSitedetailsIDOutput patient) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => AudioScreeningFormScreen(
...
class _AudioPatientRow extends StatelessWidget {
  final int index;
  final UserAttendancesUsingSitedetailsIDOutput item;
  final VoidCallback onTap;

  const _AudioPatientRow({
    required this.index,
    required this.item,
    required this.onTap,
  });
