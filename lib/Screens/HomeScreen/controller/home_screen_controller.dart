...
import 'package:s2toperational/Screens/HomeScreen/screen/update_password_dialog.dart';
...
class HomeScreenController extends GetxController {
  ...
  void _regularCampMenu() {
    if (dESGID == 34) {
      ...
      isShowRadioCamp = true;
      menuList.addAll([DashboardMenu.CampCalendar, DashboardMenu.UserAttendance]);
    } else if (dESGID == 35) {
      menuList.addAll([
        DashboardMenu.HealthScreeningDetails,
        ...
        child: AlertDialog(
          ...
          actions: [
            GetBuilder<PasswordResetController>(
              builder: (ctrl) => ctrl.isSendingOtp
                  ? const Padding(
  ...
  void pushToNextScreen(DashboardMenu dashboardMenu) {
    switch (dashboardMenu) {
      ...
      case DashboardMenu.Acknowledgement:
        Get.to(
          () => AcknowledgementCampListScreenNew(isD2D: doorToDoorCamp),
        );
        break;
