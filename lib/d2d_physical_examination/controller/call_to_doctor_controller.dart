...
import 'package:s2toperational/d2d_physical_examination/model/beneficiary_list_by_reg_id_response.dart';
...
class CallToDoctorController extends GetxController {
  ...
  int callType = 1; // 0 = not chosen, 1 = Audio, 2 = Video
  ...
  final TextEditingController callTypeTextCtrl = TextEditingController(text: "Audio");
    ...
    _api.getBeneficiaryListByRegIDAPI(
      ...
      (BeneficiaryListByRegIDResponse? response, String error, bool success) {
        if (success) {
          ...
          for (final b in beneficiaryList) {
            debugPrint(
              '[CALL_TO_DOCTOR_LOAD] regdId=${b.regdId} peStatus=${b.peStatus} '
              'doctorMappedStatus=${b.doctorMappedStatus}',
            );
          }
        } else {
          beneficiaryList = [];
  ...
  void submitAssignment(BuildContext context) {
    ...
    _api.insertBeneficiaryDoctorMappingAPI(
      ...
      (bool success, String message) {
        isSubmitting = false;
        debugPrint('[CALL_TO_DOCTOR_SUBMIT] success=$success message=$message payload=$payload');
        if (success) {
