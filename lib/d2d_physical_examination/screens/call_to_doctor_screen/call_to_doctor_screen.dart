...
import 'package:s2toperational/d2d_physical_examination/controller/call_to_doctor_controller.dart';
import 'package:s2toperational/d2d_physical_examination/model/d2d_camp_mapped_doctor_list_response.dart';
...
class CallToDoctorScreen extends StatelessWidget {
      ...
      builder:
          (_) => AlertDialog(
            ...
            content: Column(
              ...
              children: [
                _colorInfoRow(
                  color: incompleteTests1Color.withValues(alpha: 0.3),
                  label: "Doctor Assigned For Physical Exam ",
                ),
                _colorInfoRow(
                  color: ks2tBackGroundColor.withValues(alpha: 0.5),
                  label: "Physical Exam Done",
                ),
              ],
            ),
            actions: [
  ...
  Color _rowColor(String? peStatus, String? doctorMappedStatus) {
    ...
    if (doctorMappedStatus == "1") {
      return incompleteTests1Color.withValues(
        alpha: 0.3,
      ); // yellow : doctor assigned, PE pending
    }
  ...
  void _showDoctorPicker(BuildContext context, CallToDoctorController ctrl) {
    ctrl.fetchDoctorList(
      onLoaded: () {
        showModalBottomSheet(
          ...
          builder:
              (_) => SelectionBottomSheet<D2DCampMappedDoctorOutput, int>(
                title: "Choose Doctor",
  ...
  void _showCallTypePicker(BuildContext context, CallToDoctorController ctrl) {
    ...
    showModalBottomSheet(
      ...
      builder:
          (_) => SelectionBottomSheet<Map<String, dynamic>, int>(
            title: "Call Type",
            items: options,
