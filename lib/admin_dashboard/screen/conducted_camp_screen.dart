...
import 'package:s2toperational/admin_dashboard/controller/admin_controller.dart';
    ...
    return Scaffold(
      ...
      body: GetBuilder<AdminController>(
        builder: (controller) {
          ...
          if (!isLoading) {
            final List<CampRecord> allOutput =
                controller.campsConductedResponse?.output ?? <CampRecord>[];
            if (widget.title == "All Camp List") {
              final today = DateTime.now();
              rawRows = allOutput.where((camp) {
                try {
              ...
              rawRows = List<CampRecord>.from(allOutput);
            }
