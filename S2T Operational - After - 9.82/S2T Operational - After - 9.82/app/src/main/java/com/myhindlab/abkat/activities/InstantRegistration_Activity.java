...
public class InstantRegistration_Activity extends AppCompatActivity {
    ...
    private String genderId = "", title = "", TALLGDCODE;
    private int mYear, mMonth, mDay;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_instant_registration);
    ...
    private void init() {
        context = InstantRegistration_Activity.this;
...
//                }
                ...
                if (Integer.parseInt(edt_age.getText().toString().trim()) < 18) {
                    Utilities.showAlertDialog(context, "Alert", "Beneficiary age should not be less than 18 years", false);
                    return;
                }
...
//                }
                ...
                if (Utilities.isNetworkAvailable(context)) {
                    new GLO_ApprovedWorkerSiteMapping().execute(
                            SiteDetailId,
                            edt_mcobcwwbno.getText().toString().trim(),
                            "0",
                            EmpCode,
                            DISTLGDCODE,
                            title,
...
//                startActivityForResult(pickImage, CAMERA_REQUEST);
    ...
    private class GLO_ApprovedWorkerSiteMapping extends AsyncTask<String, Integer, String> {
        ...
        protected String doInBackground(String... params) {
            String res = "";
            try {
                MultipartUtility multipart = new MultipartUtility(ApplicationConstants.instantRegHandler, "UTF-8");
...
//                multipart.addFormField("DOB", params[9]);
                ...
                multipart.addFormField("CreatedBy", params[17]);
                multipart.addFormField("TALLGDCODE", params[18]);
                multipart.addFilePart("File", new File(params[19]));
...
//                    CropImage.activity(photoURI).setGuidelines(CropImageView.Guidelines.ON).start(InstantRegistration_Activity.this);
                }
            }
...
//        String destinationFilename = Environment.getExternalStorageDirectory() + "/Health Checkup/" + "Card Photos/" + filename;
    ...
    private void setUpToolbar() {
        ...
        getSupportActionBar().setTitle("Instant Registration");
