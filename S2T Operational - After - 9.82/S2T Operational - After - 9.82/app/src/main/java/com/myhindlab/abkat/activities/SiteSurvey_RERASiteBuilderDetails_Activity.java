...
import com.myhindlab.abkat.adapters.ConstructionDetailsAdapter;
import com.myhindlab.abkat.adapters.OtherPersonDetailsAdapter;
...
import com.myhindlab.abkat.models.GetListOfBuilderDetails_Responce;
...
public class SiteSurvey_RERASiteBuilderDetails_Activity extends AppCompatActivity implements View.OnClickListener {
    ...
    private EditText edt_reranumber, edt_managername, edt_managernumber;
    //    private EditText edt_buildername, edt_buildernumber, edt_builderemailid, edt_manageremailid;
    ...
    private Button btn_contractordetails, btn_otherpersondetails, btn_saveandnext, btn_next;

    private List<LinearLayout> contractorDetailsLayouts = new ArrayList<>();
    private ConstructionSitesList_Model siteDetails;

    private List<LinearLayout> otherpersonDetailsLayouts = new ArrayList<>();

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_sitesurvey_rerasitebuilderdetails);
...
//        edt_buildername = findViewById(R.id.edt_buildername);
//        edt_buildernumber = findViewById(R.id.edt_buildernumber);
//        edt_builderemailid = findViewById(R.id.edt_builderemailid);
...
//        edt_manageremailid = findViewById(R.id.edt_manageremailid);
    ...
    private void setDefaults() {
        context = SiteSurvey_RERASiteBuilderDetails_Activity.this;
...
//
//                if (edt_buildername.getText().toString().trim().isEmpty()) {
//                    edt_buildername.setError("Required Builder Name");
//                    edt_buildername.requestFocus();
//                    return;
//                }
//
//                if (edt_buildernumber.getText().toString().trim().isEmpty()) {
//                    edt_buildernumber.setError("Required Mobile Number");
//                    edt_buildernumber.requestFocus();
//                    return;
//                } else if (!Utilities.isMobileNo(edt_buildernumber.getText().toString().trim())) {
//                    edt_buildernumber.setError("Invalid Mobile Number");
//                    edt_buildernumber.requestFocus();
//                    return;
//                }
//
//                if (!edt_builderemailid.getText().toString().trim().isEmpty()) {
//                    if (!Utilities.isEmailValid(edt_builderemailid.getText().toString().trim())) {
//                        edt_builderemailid.setError("Invalid Email ID");
//                        edt_builderemailid.requestFocus();
//                        return;
//                    }
//                }
//
...
//        }
    ...
    private class GetListOfBuilderDetailsAPICall extends AsyncTask<String, Integer, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            res = WebServiceCall.APICall(ApplicationConstants.GetListOfBuilderDetails, ApplicationConstants.webservice, param);
            return res;
        }
...
//                            edt_manageremailid.setFocusable(false);
                            ...
                            bindPersonDetails(contractorDetailsList);
    ...
    private void bindPersonDetails(ArrayList<GetListOfBuilderDetails> builderDetails) {
        ArrayList<GetListOfBuilderDetails> contratorList = new ArrayList<>();
        ArrayList<GetListOfBuilderDetails> otherPersonList = new ArrayList<>();
        for (GetListOfBuilderDetails cDetails : builderDetails) {
            if (cDetails.getType().equalsIgnoreCase("1")) {
                contratorList.add(cDetails);
            } else if (cDetails.getType().equalsIgnoreCase("2")) {
                otherPersonList.add(cDetails);
            }
        }

        if (contratorList.size() > 0) {
            ...
            recycler_view_contractorList.setAdapter(new ConstructionDetailsAdapter(contratorList));
        } else {
            cd_contractordetails.setVisibility(View.GONE);
            cd_viewcontractordetails.setVisibility(View.GONE);
        }

        if (otherPersonList.size() > 0) {
            cd_otherpersondetails.setVisibility(View.GONE);
            cd_viewotherpersondetails.setVisibility(View.VISIBLE);

            LinearLayoutManager layoutManager = new LinearLayoutManager(context);
            recycler_view_otherpersonList.setLayoutManager(layoutManager);
            recycler_view_otherpersonList.setAdapter(new OtherPersonDetailsAdapter(otherPersonList));
        } else {
            cd_otherpersondetails.setVisibility(View.GONE);
            cd_viewotherpersondetails.setVisibility(View.GONE);
        }
    }
}
