...
public class CampSelectionForInstantPatientRegistration_Activity extends AppCompatActivity {
    ...
    private String subOrgId = "0";
    private int mYear, mMonth, mDay;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_campselection_for_instantpatientregistration);
...
//        };
    ...
    private void setDefaults() {
        context = CampSelectionForInstantPatientRegistration_Activity.this;
...
//                                        .putExtra("district", campDetails.getDISTNAME())
//                                        .putExtra("siteId", campDetails.getSiteDetailId()));
//                            }
//                        }));
...
//                                if (campDetails.getIsCampClosed()!=null) {
...
//                                    }
//                                }
//                                new GetUserCampMappingAndAttendanceStatus(campDetails).execute(edt_campdate.getText().toString(), userID, campList.get(position).getDISTLGDCODE(), "3", campList.get(position).getCampId());
                            }
                        }));
    ...
    private void getApprovedCampListDetails(String campDate, String subOrgId, String division, String distCode, String userId, String desgId) {
        final ProgressDialog progressDialog = new ProgressDialog(CampSelectionForInstantPatientRegistration_Activity.this); // Change to actual activity name
...
//            res = WebServiceCall.APICallWithProgress(ApplicationConstants.GetUserCampMappingAndAttendanceStatusForRegularCamp_Readiness, ApplicationConstants.webservice_d2d, param,pl);

            if (BuildConfig.isBeta) {

                res = WebServiceCall.APICall(ApplicationConstants.GetUserCampMappingAndAttendanceStatusForRegularCamp_Readiness_CampClose_V1, ApplicationConstants.webservice_d2d, param);
//             res = WebServiceCall.APICall(ApplicationConstants.GetUserCampMappingAndAttendanceStatusForRegularCamp_Readiness, ApplicationConstants.webservice_d2d, param);
...
//
                        ...
                        if (userAttendanceModel.getOutput().get(0).getCampFlag() == 0) {
                            ...
                            return;
                        }else if (userAttendanceModel.getOutput().get(0).getIsReadinessFormFilled() == 0){
                            Utilities.showAlertDialog(context, "Alert", "Readiness form is not filled. Please contact camp coordinator", false);
                            return;
                            ...
                            return;
                        } else if (userAttendanceModel.getOutput().get(0).getTestFlag() == 0 ){
                            Utilities.showAlertDialog(context, "Alert", "You are not mapped to do registration", false);
                            return;

                        }
                        else if (userAttendanceModel.getOutput().get(0).getTeamMemberAttendance() == 1 ) {
