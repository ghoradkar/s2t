...
public class CampSelectionForCampClosingList_Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_campselection_forcampclosing);
    ...
    private void setDefaults() {
        context = CampSelectionForCampClosingList_Activity.this;
                ...
                new RecyclerItemClickListener(context,
                        new RecyclerItemClickListener.OnItemClickListener() {
                            ...
                            public void onItemClick(View view, final int position) {
                                ...
                                startActivity(new Intent(context, CampClosing_Activity.class)
    ...
    private void showCampDetailsDialogBox(final GetApprovedCampListDetailsForAppList campdetails) {
        LayoutInflater layoutInflater = LayoutInflater.from(context);
        View promptView = layoutInflater.inflate(R.layout.dialog_layout_campdetails, null);
        AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(context);
        alertDialogBuilder.setTitle("Camp Details");
        alertDialogBuilder.setView(promptView);
        alertDialogBuilder.setCancelable(false);

        TextView tv_campnumber = promptView.findViewById(R.id.tv_campnumber);
        TextView tv_address = promptView.findViewById(R.id.tv_address);
        TextView tv_district = promptView.findViewById(R.id.tv_district);
        TextView tv_description = promptView.findViewById(R.id.tv_description);
        final EditText tv_remark = promptView.findViewById(R.id.tv_remark);
...
//        tv_remark.setText(campdetails.getRemark());
        ...
        }).setPositiveButton("Close Camp", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {

                if (tv_remark.getText().toString().trim().length() <= 0) {
                    Utilities.showToastMessage("Please enter remark", context, false);
                    return;
                }

                Utilities.showAlertDialog(context, "Confirmation",
                        "Are you sure you want to close " + campdetails.getCampNo() + " Camp.",
                        false, "Yes", new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                if (Utilities.isNetworkAvailable(context)) {
                                    String rem = tv_remark.getText().toString().trim();
                                    new CampClosedbyAdminAPICall().execute(campdetails.getCampId(), userID, rem);
                                } else {
                                    Utilities.showToastMessage(R.string.msgt_nointernetconnection, context, false);
                                }
                            }
                ...
                if (!result.equals("")) {
                    ...
                    if (status.equalsIgnoreCase("success")) {
                        ...
                        if (Utilities.isNetworkAvailable(context)) {
                            ...
                            Utilities.showToastMessage(R.string.msgt_nointernetconnection, context, true);
                        }
                    } else {
                        Utilities.showAlertDialog(context, status, message, false);
                    }
